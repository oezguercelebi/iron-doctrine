...
internal static class CommandIntent
{
    public static MatchOrder Create(int slot, OrderKind kind, int[] actors, int pickedId = 0,
        WorldPoint position = default, string product = "", bool append = false, int queueIndex = 0, int facing = 0)
    {
        ...
        return new MatchOrder(slot, kind, actors, target, position, product, append, queueIndex, facing);
    }
}
